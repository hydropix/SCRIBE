...
class DiscordNotifier:
    ...
    def __init__(self, config: dict):
        ...
        self.thread_name = config.get('thread_name', None)  # Optional thread name for forum channels (main webhook)
        self.summary_thread_name = summary_config.get('thread_name', None)  # Optional thread name for summary webhook
    ...
    def send_full_report(self, report_path: str, mention_role: str = "") -> bool:
        ...
        try:
            ...
            # Send first chunk and capture thread_id if created (for forum channels)
            thread_id = None
            for i, chunk in enumerate(message_chunks, 1):
                # For first message, create thread. For subsequent messages, post to thread
                success, new_thread_id = self._send_message(chunk, thread_id=thread_id)

                # Capture thread_id from first message if this is a forum channel
                if i == 1 and new_thread_id and self.thread_name:
                    thread_id = new_thread_id
                    self.logger.info(f"Created forum thread with ID: {thread_id}")
    ...
    def send_rich_report(self, relevant_contents: List[Dict[str, Any]], mention_role: str = "") -> bool:
        ...
        try:
            # Send header message first and capture thread_id if created
            header_message = self._create_header_message(len(relevant_contents), mention_role)
            success, thread_id = self._send_message(header_message)
            if not success:
                return False

            # Log thread creation for forum channels
            if thread_id and self.thread_name:
                self.logger.info(f"Created forum thread with ID: {thread_id}")
            ...
            # Send embeds for each category (posting to thread if forum channel)
            for category, contents in by_category.items():
                ...
                for content in contents:
                    ...
                    if len(embeds_batch) >= self.MAX_EMBEDS_PER_MESSAGE:
                        success, _ = self._send_embeds(embeds_batch, thread_id=thread_id)
                        if not success:
            ...
            # Send footer (posting to thread if forum channel)
            footer_message = f"---\n*Report generated by SCRIBE - {len(relevant_contents)} insights total*"
            self._send_message(footer_message, thread_id=thread_id)
    ...
    def _send_message(self, content: str, webhook_url: str = None, thread_id: str = None, thread_name: str = None) -> tuple[bool, str]:
        """
        Send a simple text message to Discord

        Args:
            content: Message content
            webhook_url: Optional webhook URL (defaults to self.webhook_url)
            thread_id: Optional thread ID to post in existing thread (for forum channels)
            thread_name: Optional thread name for creating new forum thread (overrides self.thread_name)

        Returns:
            Tuple of (success: bool, thread_id: str or None)
        """
        try:
            payload = {"content": content}

            # Use thread_id if provided (posting to existing thread)
            # Otherwise use thread_name if configured (creating new thread)
            if thread_id:
                # Posting to existing thread - add thread_id to webhook URL
                url = webhook_url or self.webhook_url
                url = f"{url}?thread_id={thread_id}"
            else:
                # Creating new thread or posting to regular channel
                # Use provided thread_name or fall back to self.thread_name
                use_thread_name = thread_name if thread_name is not None else self.thread_name
                if use_thread_name:
                    payload["thread_name"] = use_thread_name
                url = webhook_url or self.webhook_url

            response = requests.post(url, json=payload, timeout=10)

            if response.status_code in (200, 204):
                # Extract thread_id from response if this created a new thread
                try:
                    response_data = response.json() if response.text else None
                    created_thread_id = response_data.get('id') if response_data else None
                except Exception:
                    created_thread_id = None
                return True, created_thread_id
            else:
                return False, None
        except Exception as e:
    ...
    def _send_embeds(self, embeds: List[Dict[str, Any]], webhook_url: str = None, thread_id: str = None) -> tuple[bool, str]:
        """
    ...
    def send_summary(self, summary_text: str, mention_role: str = "") -> bool:
        ...
        try:
            ...
            for i, chunk in enumerate(message_chunks, 1):
                # For first message, create thread. For subsequent messages, post to thread
                success, new_thread_id = self._send_message(chunk, webhook_url=self.summary_webhook_url, thread_id=thread_id, thread_name=self.summary_thread_name)

                # Capture thread_id from first message if this is a forum channel
                if i == 1 and new_thread_id and self.summary_thread_name:
                    thread_id = new_thread_id
                    self.logger.info(f"Created summary forum thread with ID: {thread_id}")
